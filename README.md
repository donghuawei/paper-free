# 安装
1. MAC 如果用 Intelij IDE , 会给 node_modules中的文件创建索引, 用 cnpm 安装的是安装到全局, 通过link链接
 我这在创建索引时经常死机。所以不推荐直接使用 cnpm , 用下面的方式比较好
 npm install --registry=http://registry.npm.taobao.org

2. npm run dev 启动项目的时候,记得要关掉 VPN , 我这开了 VPN的话就无法访问
